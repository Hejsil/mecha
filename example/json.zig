...
fn ok(s: []const u8) !void {
    ...
    try testing.expectEqualStrings("", res.ok.rest);
}

fn err(pos: usize, s: []const u8) !void {
    try json.expectError(testing.allocator, s, pos);
}

fn errNotAllParsed(s: []const u8) !void {
    const res = json.parse(testing.allocator, s) catch @panic("test failure");
    try testing.expect(res.ok.rest.len != 0);
}
...
test "n_string_escaped_ctrl_char_tab" {
    try err(0, "\x5b\x22\x5c\x09\x22\x5d");
}

test "n_string_escaped_emoji" {
    try err(0, "[\"\x5c\xc3\xb0\xc2\x9f\xc2\x8c\xc2\x80\"]");
}
...
test "invalid continuation byte" {
    ...
    try err(0, "\"\xc4\xf5\"");
    try err(0, "\"\xc5\xff\"");
    try err(0, "\"\xe4\x80\x00\"");
    try err(0, "\"\xe5\x80\x10\"");
    try err(0, "\"\xe6\x80\xc0\"");
    try err(0, "\"\xe7\x80\xf5\"");
    try err(0, "\"\xe8\x00\x80\"");
    try err(0, "\"\xf2\x00\x80\x80\"");
    try err(0, "\"\xf0\x80\x00\x80\"");
    try err(0, "\"\xf1\x80\xc0\x80\"");
    try err(0, "\"\xf2\x80\x80\x00\"");
    try err(0, "\"\xf3\x80\x80\xc0\"");
    try err(0, "\"\xf4\x80\x80\xf5\"");
}
...
test "disallowed overlong form" {
    ...
    try err(0, "\"\xc1\x80\"");
    try err(0, "\"\xc1\x90\"");
    try err(0, "\"\xe0\x80\x80\"");
    try err(0, "\"\xf0\x80\x80\x80\"");
}

test "out of UTF-16 range" {
    try err(0, "\"\xf4\x90\x80\x80\"");
    try err(0, "\"\xf5\x80\x80\x80\"");
    try err(0, "\"\xf6\x80\x80\x80\"");
    try err(0, "\"\xf7\x80\x80\x80\"");
    try err(0, "\"\xf8\x80\x80\x80\"");
    try err(0, "\"\xf9\x80\x80\x80\"");
    try err(0, "\"\xfa\x80\x80\x80\"");
    try err(0, "\"\xfb\x80\x80\x80\"");
    try err(0, "\"\xfc\x80\x80\x80\"");
    try err(0, "\"\xfd\x80\x80\x80\"");
    try err(0, "\"\xfe\x80\x80\x80\"");
    try err(0, "\"\xff\x80\x80\x80\"");
}
