...
pub fn wrap(comptime predicate: *const fn (u8) bool) mecha.Parser(u8) {
    ...
    return .{ .parse = struct {
        fn parse(_: mem.Allocator, str: []const u8) mecha.Error!Res {
            ...
            return Res.pass(str[0], str[1..], mecha.span(0, 1));
        }
    }.parse };
}
...
pub fn not(comptime parser: anytype) mecha.Parser(u8) {
    ...
    return .{ .parse = struct {
        fn parse(allocator: mem.Allocator, str: []const u8) mecha.Error!Res {
            ...
            switch (r) {
                .err => return Res.pass(str[0], str[1..], mecha.span(0, 1)),
                .ok => {},
            }
            return Res.fail(@src(), 0);
        }
...
fn testWithPredicate(parser: anytype, pred: *const fn (u8) bool) !void {
    ...
    for (0..256) |i| {
        ...
        if (pred(c)) switch (@TypeOf(parser)) {
            mecha.Parser(u8) => try parser.match(fa, &[_]u8{c}, .{ .value = c, .span = mecha.span(0, 1) }),
            mecha.Parser(void) => try parser.match(fa, &[_]u8{c}, .{ .value = {}, .span = mecha.span(0, 1) }),
            else => comptime unreachable,
        } else switch (@TypeOf(parser)) {
            mecha.Parser(u8), mecha.Parser(void) => try parser.fail(fa, &[_]u8{c}, 0),
            else => comptime unreachable,
        }
    }
}
