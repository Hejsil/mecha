...
test "char" {
    inline for ([_]void{{}} ** 255) |_, i| {
        ...
        testWithPredicate(char(c), rangePred(c, c));
    }
}
...
test "not" {
    testWithPredicate(not(alpha), struct {
        fn pred(c: u8) bool {
...
test "digit" {
    testWithPredicate(digit(2), struct {
        fn pred(c: u8) bool {
            return switch (c) {
                '0'...'1' => true,
                else => false,
    ...
    testWithPredicate(digit(10), struct {
        fn pred(c: u8) bool {
            return switch (c) {
                '0'...'9' => true,
                else => false,
    ...
    testWithPredicate(digit(16), struct {
        fn pred(c: u8) bool {
            return switch (c) {
                '0'...'9', 'a'...'f', 'A'...'F' => true,
                else => false,
...
test "" {
    testWithPredicate(alpha, ascii.isAlpha);
    testWithPredicate(alphanum, ascii.isAlNum);
    testWithPredicate(blank, ascii.isBlank);
    testWithPredicate(cntrl, ascii.isCntrl);
    testWithPredicate(graph, ascii.isGraph);
    testWithPredicate(lower, ascii.isLower);
    testWithPredicate(print, ascii.isPrint);
    testWithPredicate(punct, ascii.isPunct);
    testWithPredicate(space, ascii.isSpace);
    testWithPredicate(upper, ascii.isUpper);
    testWithPredicate(valid, ascii.isASCII);
}
...
fn testWithPredicate(parser: anytype, pred: fn (u8) bool) void {
    ...
    for ([_]void{{}} ** 255) |_, i| {
        ...
        if (pred(c)) switch (@TypeOf(parser)) {
            mecha.Parser(u8) => mecha.expectResult(u8, .{ .value = c }, parser(allocator, &[_]u8{c})),
            mecha.Parser(void) => mecha.expectResult(void, .{ .value = {} }, parser(allocator, &[_]u8{c})),
            else => comptime unreachable,
        } else switch (@TypeOf(parser)) {
            mecha.Parser(u8) => mecha.expectResult(u8, error.ParserFailed, parser(allocator, &[_]u8{c})),
            mecha.Parser(void) => mecha.expectResult(void, error.ParserFailed, parser(allocator, &[_]u8{c})),
            else => comptime unreachable,
        }
    }
}
